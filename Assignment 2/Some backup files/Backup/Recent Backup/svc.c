...
int svc_reset(void *helper, char *commit_id) {
    ...
    Commits *commits = get_commit(helper, commit_id);
    Commits *head = NULL;

    for(int j = 0; j < my_helper->head->n_commit; ++j) {
        head = my_helper->head->head;
        if(0 == strcmp(my_helper->head-> head->commit_id, commit_id)) {
            commits = head;
            break;   
        }
