...
char *svc_merge(void *helper, char *branch_name, struct resolution *resolutions, int n_resolutions) {
    ...
    // printf("Resolution files are %d", n_resolutions);
    ...
    // for(int i = 0; i < my_branch->n_files; ++i) {
    //     printf("Name of the file in the opponents branch %s\n", my_branch->files[i].file_name);
    // }

    if(0 == check_for_uncommited_changes_merge(helper, my_branch)) {
    ...
    //print_commit(my_helper, commit_id);
    return commit_id;
    return NULL;
}
